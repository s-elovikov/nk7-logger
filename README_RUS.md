...
- Пакет включает `Runtime/csc.rsp` с `-langversion:10` для asmdef.
...
- [Конфигурация C# 10](#конфигурация-c-10)
...
- Скрипт инициализации редактора и ручной пункт меню больше не требуются.
...
## Конфигурация C# 10
- Пакет поставляется с `Runtime/csc.rsp`, где задано `-langversion:10`.
...
- C# 10 (настраивается через `Runtime/csc.rsp`)
